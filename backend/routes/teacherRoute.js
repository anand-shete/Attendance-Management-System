...
router.post('/signup', async (req, res) => {
    try {
        ...
        if (!fullname || !faculty_id || !subject || !password) return res.status(400).json({ message: "All fields are required." })

        const teacher = await Teacher.countDocuments({ faculty_id }).lean();
        if (teacher) return res.status(409).json({ message: "Account Aldready exists. Please Login!" })
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!faculty_id || !password) return res.status(400).json({ message: "All fields are required" });
...
router.get('/getQrHistory/:id', async (req, res) => {
    try {
        ...
        const qrCodeHistoryOfTeacher = await Qrcode.find({ teacherName: teacher.fullname }).sort({ createdAt: -1 }).populate('markedByStudents.student', "fullname prn");
        if (!qrCodeHistoryOfTeacher) return res.status(404).json({ message: "Generate QR Code First!" });

        return res.status(200).json(qrCodeHistoryOfTeacher);
    } catch (error) {
        console.log(error);
        return res.status(500).json({ message: "Error in fetching QR Code History" })
    }
})
...
router.post('/generateQrCode/:id', async (req, res) => {
    try {
        ...
        const qrCode = await Qrcode.create({        // create the document in db
            teacherName: teacher.fullname,
            teacherSubject: teacher.subject,
            createdByTeacher: teacherId
        })

        const qrCodeUrl = await qrcode.toDataURL(qrCode._id.toString(), { errorCorrectionLevel: 'L' });
        if (!qrCodeUrl) return res.status(500).json({ message: "Error generating QR Code" });

        await Qrcode.updateOne({ _id: qrCode._id }, { $set: { qrCodeURL: qrCodeUrl } });

        const allQrCodes = await Qrcode.find({ createdByTeacher: teacherId }).sort({ createdAt: -1 });
        if (!allQrCodes) return res.status(500).json({ message: "Error in generating QR Code" });
